...
class BaseLoader(Dataset):
    ...
    def preprocess(self, frames, bvps, config_preprocess,
                   precomputed_face_regions=None, return_face_regions=False):
        ...
        crop_result = self.crop_face_resize(
            frames,
            ...
            return_bboxes=return_face_regions,
            precomputed_face_regions=precomputed_face_regions)

        if return_face_regions:
            frames, face_regions = crop_result
        else:
            frames = crop_result
        ...
        if return_face_regions:
            return frames_clips, bvps_clips, face_regions
    ...
    def crop_face_resize(self, frames, use_face_detection, backend, use_larger_box, larger_box_coef, use_dynamic_detection,
                         ...
                         return_bboxes=False, precomputed_face_regions=None):
        ...
        if precomputed_face_regions is not None:
            face_region_all = np.asarray(precomputed_face_regions, dtype='int')
            if face_region_all.ndim == 1:
                face_region_all = face_region_all[None, :]
            if len(face_region_all) < num_dynamic_det:
                face_region_all = np.resize(face_region_all, (num_dynamic_det, 4))
        else:
            face_region_all = []
        ...
        if return_bboxes:
            return resized_frames, face_region_all
    ...
    def align_nir_to_rgb(self, rgb_frames, nir_frames, backend,
                         mode: str = "first_frame",
                         fixed_M=None,
                         save_csv_path=False,
                         return_debug: bool = False):
        """NIR 프레임을 RGB 좌표계로 warp합니다 (face landmark 기반 homography).

        Args:
            rgb_frames : (T, H, W, 3)  RGB 프레임 배열
            nir_frames : (T, H, W, 1) 또는 (T, H, W) NIR 프레임 배열
            backend    : 얼굴 검출 백엔드 문자열 (Y5F-s 등)
            mode       : "fixed" — fixed_M 사용 / "first_frame" — 첫 프레임에서 추정
            fixed_M    : (2×3) 또는 (3×3) 변환 행렬, mode="fixed" 일 때 사용
            save_csv_path, return_debug : 호환용 파라미터 (미사용)

        Returns:
            np.ndarray (T, H_rgb, W_rgb, C) — RGB 해상도로 warp된 NIR 프레임
        """
        if "Y5F" not in backend.upper() and "YOLOFACE" not in backend.upper():
            return nir_frames

        if not hasattr(self, "Y5FObj") or self.Y5FObj is None:
            from dataset.data_loader.face_detector.YOLO5Face import YOLO5Face
            self.Y5FObj = YOLO5Face(backend)
        detector = self.Y5FObj
        ...
        def to_bgr_for_det(frame):
            f = to_uint8(frame)
            if f.ndim == 2:
                return cv2.cvtColor(np.repeat(f[:, :, None], 3, axis=2), cv2.COLOR_RGB2BGR)
            if f.shape[2] == 1:
                return cv2.cvtColor(np.repeat(f, 3, axis=2), cv2.COLOR_RGB2BGR)
            return cv2.cvtColor(f, cv2.COLOR_RGB2BGR)

        T = min(len(rgb_frames), len(nir_frames))
        if T == 0:
            return nir_frames

        H_rgb, W_rgb = rgb_frames[0].shape[:2]
        nir_is_gray = (nir_frames[0].ndim == 2) or (nir_frames[0].shape[-1] == 1)

        M_use = None
        if mode == "fixed" and fixed_M is not None:
            M_use = np.asarray(fixed_M, dtype=np.float32)

        if M_use is None:
            res_rgb = detector.detect_face(to_bgr_for_det(rgb_frames[0]), return_landmarks=True)
            res_nir = detector.detect_face(to_bgr_for_det(nir_frames[0]), return_landmarks=True)
            if not res_rgb or not res_nir:
                return nir_frames
            lm_rgb = res_rgb.get("landmarks")
            lm_nir = res_nir.get("landmarks")
            if lm_rgb is None or lm_nir is None:
                return nir_frames
            n_pts = min(len(lm_nir), len(lm_rgb))
            if n_pts < 4:
                return nir_frames
            src = lm_nir[:n_pts].astype(np.float32)
            dst = lm_rgb[:n_pts].astype(np.float32)
            method = cv2.RANSAC if n_pts >= 8 else 0
            M_use, _ = cv2.findHomography(src, dst, method)
            if M_use is None:
                return nir_frames
            M_use = M_use.astype(np.float32)

        C_out = 1 if nir_is_gray else 3
        out = np.empty((T, H_rgb, W_rgb, C_out), dtype=nir_frames.dtype)
        for t in range(T):
            f_bgr = to_bgr_for_det(nir_frames[t])
            if M_use.shape == (3, 3):
                warped = cv2.warpPerspective(f_bgr, M_use, (W_rgb, H_rgb))
            else:
                warped = cv2.warpAffine(f_bgr, M_use, (W_rgb, H_rgb))
            if nir_is_gray:
                warped = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)[:, :, None]
            out[t] = warped.astype(nir_frames.dtype)
