...
METRIC_COLUMNS = ['MAE', 'RMSE', 'MAPE', 'Pearson', 'SNR', 'MACC']


def save_metrics_to_excel(metrics_dict, filename_id, config, eval_method, n_samples):
    """Append a row of metric results to a CSV file.

    metrics_dict: {metric_name: (mean, se)} for each computed metric.
    One CSV file per filename_id; each run appends a new row.
    """
    save_dir = config.TEST.EXCEL_SAVE_DIR
    os.makedirs(save_dir, exist_ok=True)
    csv_path = os.path.join(save_dir, f'{filename_id}.csv')

    header = ['seed', 'timestamp', 'eval_method', 'n_samples']
    for m in METRIC_COLUMNS:
        header += [f'{m}_mean', f'{m}_se']
    ...
    for m in METRIC_COLUMNS:
        if m in metrics_dict:
            mean_val, se_val = metrics_dict[m]
            row += [mean_val, se_val]
        else:
            row += [None, None]
...
def calculate_metrics(predictions, labels, config):
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        if num_test_samples == 0:
            print("No test samples found — check that the test data path and split fractions are correct.")
            return
        fft_metrics = {}
        for metric in config.TEST.METRICS:
            if metric == "MAE":
                ...
                fft_metrics['MAE'] = (MAE_FFT, standard_error)
            elif metric == "RMSE":
                ...
                fft_metrics['RMSE'] = (RMSE_FFT, standard_error)
            elif metric == "MAPE":
                ...
                fft_metrics['MAPE'] = (MAPE_FFT, standard_error)
            elif metric == "Pearson":
                Pearson_FFT = np.corrcoef(predict_hr_fft_all, gt_hr_fft_all)
                correlation_coefficient = Pearson_FFT[0][1]
                standard_error = np.sqrt((1 - correlation_coefficient**2) / (num_test_samples - 2))
                print("FFT Pearson (FFT Label): {0} +/- {1}".format(correlation_coefficient, standard_error))
                fft_metrics['Pearson'] = (correlation_coefficient, standard_error)
            elif metric == "SNR":
                SNR_FFT = np.mean(SNR_all)
                standard_error = np.std(SNR_all) / np.sqrt(num_test_samples)
                print("FFT SNR (FFT Label): {0} +/- {1} (dB)".format(SNR_FFT, standard_error))
                fft_metrics['SNR'] = (SNR_FFT, standard_error)
            elif metric == "MACC":
                ...
                fft_metrics['MACC'] = (MACC_avg, standard_error)
            elif "AU" in metric:
                pass
            elif "BA" in metric:
        ...
        save_metrics_to_excel(fft_metrics, filename_id, config, 'FFT', num_test_samples)
        ...
        for metric in config.TEST.METRICS:
            if metric == "MAE":
                ...
                peak_metrics['MAE'] = (MAE_PEAK, standard_error)
            elif metric == "RMSE":
                # Calculate the squared errors, then RMSE, in order to allow
                # for a more robust and intuitive standard error that won't
                # be influenced by abnormal distributions of errors.
                ...
                peak_metrics['RMSE'] = (RMSE_PEAK, standard_error)
            elif metric == "MAPE":
                ...
                peak_metrics['MAPE'] = (MAPE_PEAK, standard_error)
            elif metric == "Pearson":
                ...
                peak_metrics['Pearson'] = (correlation_coefficient, standard_error)
            elif metric == "SNR":
                SNR_PEAK = np.mean(SNR_all)
                standard_error = np.std(SNR_all) / np.sqrt(num_test_samples)
                print("PEAK SNR (PEAK Label): {0} +/- {1} (dB)".format(SNR_PEAK, standard_error))
                peak_metrics['SNR'] = (SNR_PEAK, standard_error)
            elif metric == "MACC":
                MACC_avg = np.mean(MACC_all)
                standard_error = np.std(MACC_all) / np.sqrt(num_test_samples)
                print("PEAK MACC (PEAK Label): {0} +/- {1}".format(MACC_avg, standard_error))
                peak_metrics['MACC'] = (MACC_avg, standard_error)
            elif "AU" in metric:
                pass
            elif "BA" in metric:
                ...
                raise ValueError("Wrong Test Metric Type")
        save_metrics_to_excel(peak_metrics, filename_id, config, 'peak detection', num_test_samples)
    else:
        raise ValueError("Inference evaluation method name wrong!")
