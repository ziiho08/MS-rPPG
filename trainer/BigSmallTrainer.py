...
class BigSmallTrainer(BaseTrainer):
    ...
    def save_model(self, index):
        ...
        model_path = os.path.join(self.model_dir, self.model_file_name + '_Epoch' + str(index) + '_Seed' + str(self.seed) + '.pth')
        torch.save(self.model.state_dict(), model_path)
        print('Saved Model Path: ', model_path)
        print('')
        if index > 0:
            prev_path = os.path.join(self.model_dir, self.model_file_name + '_Epoch' + str(index - 1) + '_Seed' + str(self.seed) + '.pth')
            keep_prev = (not self.config.TEST.USE_LAST_EPOCH) and (index - 1 == self.used_epoch)
    ...
    def test(self, data_loader):
        ...
        # IF USING MODEL FROM TRAINING
            ...
            model_path = os.path.join(self.model_dir,
                                           self.model_file_name + '_Epoch' + str(self.used_epoch) + '_Seed' + str(self.seed) + '.pth')
        ...
        if self.config.TEST.DATA.DATASET.upper() == 'PHYSDRIVE':
            ...
            calculate_metrics_by_condition(preds_dict_bvp, labels_dict_bvp, self.config)
