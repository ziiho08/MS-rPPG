...
def add_args(parser):
    ...
    parser.add_argument('--seed', required=False, default=None, type=int, help="Override SEED in config.")
...
def train_and_test(config, data_loader_dict):
    ...
    elif config.MODEL.NAME == 'MSMamba':
        model_trainer = trainer.MSMambaTrainer.MSMambaTrainer(config, data_loader_dict)
    else:
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        elif config.TRAIN.DATA.DATASET == "MSDrive":
            train_loader = data_loader.MSDriveLoader.MSDriveLoader
        else:
        ...
        elif config.VALID.DATA.DATASET == "MSDrive":
            valid_loader = data_loader.MSDriveLoader.MSDriveLoader
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        elif config.TEST.DATA.DATASET == "MSDrive":
            test_loader = data_loader.MSDriveLoader.MSDriveLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS and iBVP.")
        ...
        elif config.UNSUPERVISED.DATA.DATASET == "PhysDrive":
            unsupervised_loader = data_loader.PhysDriveLoader.PhysDriveLoader
        elif config.UNSUPERVISED.DATA.DATASET == "MR-NIRP":
            unsupervised_loader = data_loader.MRNIRPLoader.MRNIRPLoader
        elif config.UNSUPERVISED.DATA.DATASET == "MSDrive":
            unsupervised_loader = data_loader.MSDriveLoader.MSDriveLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
