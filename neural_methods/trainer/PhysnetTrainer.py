    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                if self.config.TRAIN.DATA.DATA_FORMAT == 'NDCHW':
                ...
                BVP_label = (BVP_label - torch.mean(BVP_label)) / \
                            (torch.std(BVP_label))
                loss = self.loss_model(rPPG, BVP_label) # + 1e-8
                loss.backward()
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                BVP_label = valid_batch[1].to(
                    torch.float32).to(self.device)
                valid_data = valid_batch[0].to(torch.float32).to(self.device)
                if self.config.VALID.DATA.DATA_FORMAT == 'NDCHW':
                    valid_data = valid_data.permute(0, 2, 1, 3, 4)
                rPPG, x_visual, x_visual3232, x_visual1616 = self.model(valid_data)
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                if self.config.TEST.DATA.DATA_FORMAT == 'NDCHW':
                    data = data.permute(0, 2, 1, 3, 4)
